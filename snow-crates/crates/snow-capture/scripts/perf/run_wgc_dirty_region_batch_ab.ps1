...
$baselineCsv = Join-Path $outputPath "wgc_dirty_batch_baseline.csv"
...
try {
    ...
    $baselineBatchFetchEnvName = "SNOW_CAPTURE_WGC_DISABLE_DIRTY_REGION_BATCH_FETCH"
    $hadBaselineBatchFetchValue = Test-Path ("Env:{0}" -f $baselineBatchFetchEnvName)
    $baselineBatchFetchValue = if ($hadBaselineBatchFetchValue) {
        [System.Environment]::GetEnvironmentVariable($baselineBatchFetchEnvName, "Process")
    } else {
        $null
    }
    ...
    try {
        ...
        if ($hadBaselineBatchFetchValue) {
            $env:SNOW_CAPTURE_WGC_DISABLE_DIRTY_REGION_BATCH_FETCH = $baselineBatchFetchValue
        }
        else {
    ...
    Write-Host ("A/B result vs baseline (dirty-region batch fetch disabled): avg={0:N2}% p50={1:N2}% p95={2:N2}%" -f $avgImprovementPct, $p50ImprovementPct, $p95ImprovementPct)
