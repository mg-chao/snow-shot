...
vec3 linearToSrgb(vec3 value)
{
    ...
    vec3 highValue = 1.055 * pow(max(value, vec3(0.0)), vec3(1.0 / 2.4)) - 0.055;
...
vec3 srgbToLinear(vec3 value)
{
    ...
    vec3 highValue = pow((max(value, vec3(0.0)) + 0.055) / 1.055, vec3(2.4));
    return mix(highValue, value / 12.92, low);
}
...
vec4 sampleNearestPixel(ivec2 targetPixel, ivec2 targetDimensions)
{
    ...
    return prepareSample(texelFetch(sourceTexture, texel, 0));
}
...
vec4 fetchSource(ivec2 texel, int mipLevel)
{
    ivec2 dimensions = max(
        ivec2(uniforms.sourceAndTarget.xy + vec2(0.5)), ivec2(1));
    texel = clamp(texel, ivec2(0), dimensions - ivec2(1));
    if (uniforms.backendParameters.y > 0.5)
        texel.y = dimensions.y - 1 - texel.y;
    return prepareSample(texelFetch(sourceTexture, texel, mipLevel));
}
                     ...
                     float mipLevel)
{
    ...
    for (int y = -2; y <= 3; ++y) {
        ...
        for (int x = -2; x <= 3; ++x) {
            ...
            ivec2 texel = clamp(base + ivec2(x, y), ivec2(0), ivec2(dimensions) - ivec2(1));
            total += fetchSource(texel, int(mipLevel + 0.5)) * weight;
            totalWeight += weight;
        }
    }
                       ...
                       float mipLevel, int axis)
{
    ...
    for (int tap = -2; tap <= 3; ++tap) {
        ...
        ivec2 texel = clamp(base + offset, ivec2(0), ivec2(dimensions) - ivec2(1));
        total += fetchSource(texel, int(mipLevel + 0.5)) * weight;
        totalWeight += weight;
    }
    return total / max(totalWeight, 0.00001);
...
vec4 sampleLinear(ivec2 targetPixel, ivec2 targetDimensions)
{
    ...
    vec2 fraction = sourcePosition(
        targetPixel, targetDimensions, dimensions, base);
    vec4 top = mix(fetchSource(base, 0),
                   fetchSource(base + ivec2(1, 0), 0), fraction.x);
    vec4 bottom = mix(fetchSource(base + ivec2(0, 1), 0),
                      fetchSource(base + ivec2(1, 1), 0), fraction.x);
...
void main()
{
    ...
    else value = sampleNearestPixel(targetPixel, targetDimensions);
